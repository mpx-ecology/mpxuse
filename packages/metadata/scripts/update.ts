...
import type { PackageIndexes, MpxUseFunction, MpxUsePackage } from './types'
...
export const DOCS_URL = 'https://mpxuse.org'
...
export async function readMetadata() {
  ...
  for (const info of packages) {
    ...
    await Promise.all(functions.map(async (fnName) => {
      ...
      if (typeof alias === 'string') alias = alias.split(',').map(s => s.trim()).filter(Boolean)
      let related = frontmatter.related
      if (typeof related === 'string') { related = related.split(',').map(s => s.trim()).filter(Boolean) } else if (Array.isArray(related)) related = related.map(s => s.trim()).filter(Boolean)
